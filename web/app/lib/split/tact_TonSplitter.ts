...
function dictValueParserSplitTons(): DictionaryValue<SplitTons> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeSplitTons(src)).endCell());
        },
        parse: (src) => {
            return loadSplitTons(src.loadRef().beginParse());
        }
    }
}

export type TonSplitter$Data = {
    $$type: 'TonSplitter$Data';
    owner: Address;
}

export function storeTonSplitter$Data(src: TonSplitter$Data) {
    return (builder: Builder) => {
        let b_0 = builder;
...
export function loadTonSplitter$Data(slice: Slice) {
    let sc_0 = slice;
    let _owner = sc_0.loadAddress();
    return { $$type: 'TonSplitter$Data' as const, owner: _owner };
}

function loadTupleTonSplitter$Data(source: TupleReader) {
    let _owner = source.readAddress();
    return { $$type: 'TonSplitter$Data' as const, owner: _owner };
}

function loadGetterTupleTonSplitter$Data(source: TupleReader) {
    let _owner = source.readAddress();
    return { $$type: 'TonSplitter$Data' as const, owner: _owner };
}

function storeTupleTonSplitter$Data(source: TonSplitter$Data) {
    let builder = new TupleBuilder();
    builder.writeAddress(source.owner);
    return builder.build();
}

function dictValueParserTonSplitter$Data(): DictionaryValue<TonSplitter$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeTonSplitter$Data(src)).endCell());
        },
        parse: (src) => {
            return loadTonSplitter$Data(src.loadRef().beginParse());
        }
    }
}

 type TonSplitter_init_args = {
    $$type: 'TonSplitter_init_args';
    owner: Address;
    randomId: bigint;
}

function initTonSplitter_init_args(src: TonSplitter_init_args) {
    return (builder: Builder) => {
        let b_0 = builder;
        b_0.storeAddress(src.owner);
...
async function TonSplitter_init(owner: Address, randomId: bigint) {
    ...
    initTonSplitter_init_args({ $$type: 'TonSplitter_init_args', owner, randomId })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const TonSplitter_errors: { [key: number]: { message: string } } = {
...
const TonSplitter_types: ABIType[] = [
...
const TonSplitter_getters: ABIGetter[] = [
    {"name":"owner","arguments":[],"returnType":{"kind":"simple","type":"address","optional":false}},
]

export const TonSplitter_getterMapping: { [key: string]: string } = {
    'owner': 'getOwner',
}

const TonSplitter_receivers: ABIReceiver[] = [
    {"receiver":"internal","message":{"kind":"typed","type":"SplitTons"}},
    {"receiver":"internal","message":{"kind":"text","text":"withdraw"}},
    {"receiver":"internal","message":{"kind":"typed","type":"Deploy"}},
]

export class TonSplitter implements Contract {
    
    static async init(owner: Address, randomId: bigint) {
        return await TonSplitter_init(owner, randomId);
    }
    
    static async fromInit(owner: Address, randomId: bigint) {
        const init = await TonSplitter_init(owner, randomId);
        const address = contractAddress(0, init);
        return new TonSplitter(address, init);
    }
    
    static fromAddress(address: Address) {
        return new TonSplitter(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  TonSplitter_types,
        getters: TonSplitter_getters,
        receivers: TonSplitter_receivers,
        errors: TonSplitter_errors,
    };
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: SplitTons | 'withdraw' | Deploy) {
        
        let body: Cell | null = null;
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'SplitTons') {
            body = beginCell().store(storeSplitTons(message)).endCell();
        }
        if (message === 'withdraw') {
